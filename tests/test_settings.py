...
class TestSettings:
    ...
    def test_drf_in_installed_apps(self, app):
        ...
        assert app in settings.INSTALLED_APPS, (
            f'`{app}` отсутствует в `INSTALLED_APPS` в '
            'настройках приложения. '
            'Убедитесь, что необходимые модули '
            'Django REST Framework добавлены в '
            '`INSTALLED_APPS` в настройках проекта.'
        )
    ...
    def test_api_in_installed_apps(self):
        ...
        assert {'api', 'api.apps.ApiConfig'}.intersection(
            ...
            'Убедитесь, что приложение `api` добавлено в '
            '`INSTALLED_APPS` в настройках проекта.'
        )
